...
class SignUpViewModel @Inject constructor(val firebaseAuthInteractor: FirebaseAuthInteractor) :
  ...
  private fun checkIfEmailIsValid() {
    if (Patterns.EMAIL_ADDRESS.matcher(_email.value).matches()) {
      viewModelScope.launch {
        val emailExists = checkIfEmailIsAlreadyRegistered()
        _validEmail.value = !emailExists
      }
    } else {
  ...
  private suspend fun checkIfEmailIsAlreadyRegistered(): Boolean {
    return firebaseAuthInteractor.checkIfEmailExists(_email.value)
  }
